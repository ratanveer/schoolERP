...
class Studentfeemaster_model extends CI_Model {
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null, $address = null) {
        $sql = "SELECT IFNULL(`student_fees_master`.`id`, '0') as `student_fees_master_id`,`classes`.`id` AS `class_id`,"
				...
				//. "ON `student_session`.`student_id` = `students`.`permanent_address` "
				//. "ON `students`.`permanent_address` = `student_session`.`id` LEFT JOIN student_fees_master on"
			...
			//$sql = "SELECT * FROM students WHERE permanent_address LIKE '".$address."'";
        if ($class_id != null) {
		...
		if ($address != null) {
            ...
            $sql .= " AND `students`.`permanent_address` =" . $this->db->escape($address);
