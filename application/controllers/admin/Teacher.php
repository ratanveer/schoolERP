...
class Teacher extends Admin_Controller {
	...
	function exportformat() {
        $array = array(
            array("name", "email",
                "password", "address",
                "dob(YYYY-MM-DD)", "designation",
                "sex", "phone","image","is_active(Yes/No)"),
        );
        $this->load->helper('csv');
        echo array_to_csv($array, 'import_teacher_sample_file.csv');
    }
	...
	function teacherimport() {
		...
		$this->session->set_userdata('top_menu', 'Teacher Information');
        $this->session->set_userdata('sub_menu', 'teacher/teacherimport');
       $session = $this->setting_model->getCurrentSession();
        ...
        if ($this->form_validation->run() == False) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/teacher/teacherimport', $data);
            $this->load->view('layout/footer', $data);
        } else {	        
           $session = $this->setting_model->getCurrentSession();
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                for ($i = 1; $i <= count($result); $i++){				
                    ...
                    $insert_id = $this->teacher_model->add($result[$i]);
          ...
          $this->session->set_flashdata('msg', '<div student="alert alert-success text-center">Employee imported successfully</div>');
           redirect('admin/teacher/teacherimport');
        }
    }
